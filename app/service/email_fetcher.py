...
class EmailFetcher:
    ...
    def __init__(
        ...
        if output_dir:
            ...
            # По умолчанию: data/CVs относительно корня проекта
    ...
    def _generate_unique_filename(self, filename: str) -> str:
        ...
        # Добавляем timestamp если файл существует
    ...
    def _save_text_cv(self, text: str, subject: str, sender: str) -> Optional[str]:
        ...
        if not text or len(text.strip()) < 100:  # Минимальная длина для резюме
            return None
        
        # Создаем имя файла из темы и отправителя
        ...
        try:
            ...
            for email_id in email_ids:
                try:
                    ...
                    # Сохраняем текстовое резюме если нет вложений
                    if save_text_body and not has_attachment and text_body:
                        # Проверяем, похоже ли письмо на резюме
