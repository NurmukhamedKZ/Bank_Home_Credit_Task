...
## Метрики
- Сделали Датасет с 5 вакансиями и 25 резюме для высчитывания метрик
- Лучшим способом поиска оказался просто dense search с voyage-4-large
- Метррики: MAP, MRR, Precision@K, Recall@K, NDCG, mean_latency

---

## Полный пайплайн

Резюме в email -> мониторин скачивает файл -> сортировка по папкам (pdf, docx, txt) -> парсинг -> сохранение текста в data/Parsed_CVs -> структурирование в JSON файл -> сохранение в JSON_CVs -> переобучение TF-IDF модели на новых резюме -> создания векторов и сохранение в QDrant
